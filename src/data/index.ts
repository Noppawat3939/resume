...
export const works: {
...
}[] = [
  {
    ...
    tasks: [
      "Developed and maintained Trade-in platform, including an image annotation tool that reduced phone inspection time by over 30%, implementing localization support for multi-country usage, and building a deposit feature integrated with payment services such as M-Pay and Shopee Pay Later, using Next.js.",
      ...
      "Developed backend services for both Trade-in and Admin systems within a monolithic backend architecture, contributing to a shared backend monorepo that supports multiple frontend applications by developing internal and partner-facing APIs based on defined API specifications, designing databases for new features, optimizing queries for improved performance using Node.js (Express), Sequelize, PostgreSQL, Redis, and Firebase Realtime Database.",
      ...
      "Developed a End-user web application for mobile phone inspection and second-hand trade-in, supporting the company’s expansion into the Malaysia market by integrating device grading and pricing APIs, enabling localized and mobile-responsive experiences to attract new trade-in customers, using Next.js.",
      "Developed a monolithic Service-Center backend system from scratch, building APIs for repair orders, quotations, invoices, payment processing, and inventory management using NestJS, TypeORM, and PostgreSQL.",
      "Developed a Trade-in member web application on LINE LIFF, focusing on the frontend to enable users to log in via LINE and redeem points seamlessly within the LINE ecosystem, using Next.js integrated with LINE LIFF.",
